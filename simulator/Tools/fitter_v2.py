...
.. sectionauthor:: Thomas Cokelaer, Aug 2014-2020
   Modification::  David Poves, Jun 2020
...
class Fitter(object):
    ...
    def __init__(self, x_data, y_data, **kwargs):
        """
        Initialize the Fitter class with the desired parameters.

        Parameters
        ----------
        x_data : numpy array/list
            Array/list of the x data.
        y_data : numpy array/list
            Array/list of the y data..
        **kwargs : A list of all possible keyword arguments are
            DESCRIPTION.
        ...
        # Define all possible keyword arguments.
        valid_keys = ['xmin', 'xmax', 'bins', 'distributions', 'verbose',
                      'timeout', 'density']
        keys = [key for key in kwargs]
        for key in keys:
            # Check for any non valid keyword argument.
            if not key in valid_keys:
                raise KeyError(f'{key} is not a valid keyword argument. Valid kwargs are {valid_keys}')
