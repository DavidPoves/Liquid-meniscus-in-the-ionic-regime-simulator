...
class LiquidProperties(object):
    ...
    def __init__(self, liquid, relative_permittivity=10):
        ...
        if liquid == 'EMIBF4':  # Complete name: 1-ethyl-3-methylimidazolium Tetrafluoroborate.

            self.liquid_used = 'EMIBF4'  # To let the user know which liquid was used during the simulation.
    ...
    def check_available_liquids():
        """
        Check which liquids are available for the user to select from. This list must be updated by the programmer if
        a new liquid is used.
        Returns:

        """
        avail_liquids = ['EMIBF4']
        print(avail_liquids)
